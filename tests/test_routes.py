...
def test_internal_server_error(flask_test_client):
    ...
    try:
        with patch("app.blueprints.fund_builder.routes.get_fund_by_id") as mock_get_fund_by_id:
            mock_get_fund_by_id.side_effect = Exception("Trigger 500 error")
            response = flask_test_client.get("/fund/view?fund_id=99999")

...
@pytest.fixture
def test_section():
    """Fixture that creates a test section with default values."""
    return insert_new_section(
        {
            "name_in_apply_json": {"en": "Test section name"},
            "index": 1,
        }
    )
...
def test_delete_section(flask_test_client, _db, clear_test_data, seed_dynamic_data, test_section):
    assert isinstance(test_section, Section)
    test_section_id = test_section.section_id
    delete_section(test_section.section_id)
    assert _db.session.query(Section).filter(Section.section_id == test_section_id).one_or_none() is None


def test_delete_section_with_full_cascade(
    flask_test_client, _db, clear_test_data, seed_dynamic_data, test_section, test_form
):
    assert isinstance(test_section, Section)
    test_section_id = test_section.section_id
    ...
    test_form_id = test_form.form_id
    test_form.section_id = test_section_id

    # Should successfully delete everything with cascade=True
    delete_section(test_section_id, cascade=True)
    assert _db.session.query(Section).where(Section.section_id == test_section_id).one_or_none() is None
    assert _db.session.query(Form).where(Form.form_id == test_form_id).one_or_none() is None
...
def test_failed_delete_section_with_fk_to_forms(
    ...
    # Should fail to delete with cascade=False
    with pytest.raises(IntegrityError):
        delete_section(test_section_id, cascade=False)
    _db.session.rollback()
    assert _db.session.query(Section).filter(Section.section_id == test_section_id).one_or_none() is not None
    assert _db.session.query(Form).where(Form.form_id == test_form_id).one_or_none() is not None
...
def test_insert_new_form(flask_test_client, _db, clear_test_data, seed_dynamic_data):
    ...
    assert new_form.form_json == {}


def test_update_form(flask_test_client, _db, clear_test_data, seed_dynamic_data, test_form):
    assert isinstance(test_form, Form)
    ...
    assert test_form.form_json == {}

    updated_form: Form = update_form(
        form_id=test_form.form_id,
        form_name="Updated form name",
        template_name="Updated template name",
        runner_publish_name="updated-template-name",
        form_json={"is_upated": True},
    )
    ...
    assert updated_form.runner_publish_name == "updated-template-name"
    assert updated_form.form_json == {"is_upated": True}


def test_delete_form(flask_test_client, _db, clear_test_data, seed_dynamic_data, test_form):
    assert isinstance(test_form, Form)

...
def get_db_revision_id() -> str | None:
    ...
    try:
        grep_result = subprocess.check_output(
            (
                "copilot svc exec --name fsd-fund-application-builder --yes false "
...
def init_task() -> str:
    try:
        task_command = subprocess.run(
            args=["copilot", "task", "run", "--generate-cmd", f"pre-award/{environment}/fsd-fund-application-builder"],
            capture_output=True,
            check=True,
            text=True,
        )
...
def get_current_alembic_head_file_content() -> str:
    ...
    with open("app/db/migrations/.current-alembic-head", "r") as file:
